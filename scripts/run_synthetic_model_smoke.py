#!/usr/bin/env python3
"""Run a deterministic synthetic smoke test of one configured HAR model."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Exercise a configured conservative HAR model on generated arrays only. "
            "This command cannot produce a scientific result or access the external "
            "hold-out cohort."
        )
    )
    parser.add_argument(
        ...
        help="Compact-CNN or compact-TCN configuration file.",
    )
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument(
        "--predictions-output",
        type=Path,
        help="Optional immutable validation-logit artefact written after training.",
    )
...
def main() -> int:
    """Execute the synthetic smoke test and write immutable result artefacts."""
    ...
    record = build_training_result_record(
        run_id=f"synthetic-{configuration.experiment_id}-smoke-{arguments.seed}",
    ...
    if arguments.predictions_output is not None:
        ...
        prediction_record = build_prediction_artifact_record(
            run_id=f"synthetic-{configuration.experiment_id}-predictions-{arguments.seed}",
            created_at_utc=created_at.isoformat().replace("+00:00", "Z"),
            logits=logits,
